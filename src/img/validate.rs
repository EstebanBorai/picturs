...
fn validate(dim: &'a Dimensions) -> Result<(), IconizeError> {
  if dim.height != dim.width {
    return Err(IconizeError::new(&format!(
...
fn validate_macos(dim: &'a Dimensions) -> Result<(), IconizeError> {
  if dim.width < 512 {
...
fn validate_linux(dim: &'a Dimensions) -> Result<(), IconizeError> {
  if dim.width < 16 {
    return Err(IconizeError::new(
...
fn validate_windows(dim: &'a Dimensions) -> Result<(), IconizeError> {
  if dim.width < 256 {
    return Err(IconizeError::new(
